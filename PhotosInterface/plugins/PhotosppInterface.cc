    ...
    setAbsoluteInfraredCutOff_(false)
{
  ...
  for (unsigned int ip=0; ip<par.size(); ++ip ){
    ...
    if(curSet=="setAbsoluteInfraredCutOff") setAbsoluteInfraredCutOff_ = fPSet->getParameter<bool>(curSet);
  }
...
HepMC::GenEvent* PhotosppInterface::apply( HepMC::GenEvent* evt){
  ...
  if(lhe!=nullptr){ 
    ...
    // add lepton four-vector to calculate DY mass 
    HepMC::GenParticle dilep;
    double Px = particles.at(0).momentum().px()+particles.at(1).momentum().px();
    double Py = particles.at(0).momentum().py()+particles.at(1).momentum().py();
